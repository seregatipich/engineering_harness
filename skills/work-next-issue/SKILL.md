...
description: >-
  ...
  partition the work by dependencies and file overlap, post one executable Brief
  per issue, implement test-first (parallel worktrees where the partition
  allows), verify independently, and merge to the integration branch serially.
when_to_use: >-
...
One run takes the scoped set of issues zero to hero: select → provision the environment and baseline → research (subagents) → partition (parallel / pipelined / serialized) → one Master Plan → one executable Brief posted per issue → implement test-first (one worktree-isolated subagent per issue, concurrent where the partition allows) → verify and merge serially.
...
1. **Interpret the request into selector flags.** `$ARGUMENTS` is free-form — empty, explicit `#numbers`, or a sentence. The scope is the user's: work exactly what they describe, no more. The selector parses flags, never prose, so translate here, by the decision rules, never by asking:
   * explicit issue references → `#42 #43`
   * one or more milestones, by title or number → repeat `--milestone` per milestone
   * a priority ("critical", "high priority") → `--priority <critical|high|medium|low>` (a floor: that level and everything more urgent)
   * an issue type or label → `--label <name>` for each; if nothing matches, drop the filter and record it rather than inventing a label
   * an explicit count → a bare trailing number, an exact ceiling. Only when the user names a quantity; there is no default. Do not turn an open-ended request into a number.

   Record every word→flag mapping as a numbered assumption citing Rule 1. Empty arguments → no flags: the fully unscoped default.
2. `bash "${CLAUDE_SKILL_DIR}/scripts/select_batch.sh" <owner/repo> <flags>` → JSON with `cap`, `filters`, `label_map`, `assumptions`, and the ordered `batch`. **Read `label_map`**: it reports which of this repo's real labels play the priority, in-progress and awaiting-release roles. A role with no match means that guard is inert — say so in the Master Plan rather than assuming it works. Selection: explicit issues first (filters don't apply to them), then eligible open issues filled by nearest-due milestone then priority. A cap is a ceiling, never a target.
3. `bash "${CLAUDE_SKILL_DIR}/scripts/setup_labels.sh" <owner/repo>` with that map — it creates a label only for a role the repo has none for, so it never imposes a second vocabulary.
4. **Claim immediately:** add the in-progress role label to every selected issue. This is what stops a concurrent run picking up the same work.
5. Report the batch as a table (#, title, milestone, priority, order) and proceed without waiting for acknowledgment.
...
* Write each issue's **Brief** with `references/handoff.md`, post it on its issue, and record its comment id. The Brief is the whole per-issue contract; there is no second, richer document.
...
**The dispatch prompt is the posted Brief, fetched verbatim, plus the machine block from `scripts/agent_env.sh <n> <worktree>` — nothing else is authored.** To tell an executor something the Brief lacks, PATCH the comment and re-fetch. Pass the Agent tool's per-invocation `model` on implementation dispatches only; research subagents get none and inherit the orchestrator. If the preloaded Executor tier line is set, that env var outranks the parameter — record the effective tier as an assumption and pass no conflicting `model`. Beyond 20 concurrent subagents, dispatches queue; that is not an error.
...
1. **Verify in place, and never take the subagent's word.** The subagent's worktree already has the dependencies and the resource slot, so verify there: run `VERIFY`, compare against the baseline with `run_units.sh --baseline`, read the diff adversarially, and drive the behaviour end to end. The end-to-end drive belongs to this context; the executor's obligation was to turn any e2e path the Brief named into a committed test, not a throwaway probe.
...
5. **Ship:** post the closing comment, swap the in-progress role label for awaiting-release. Do **not** close the issue — closing happens at promotion. Tick the checklist.
...
* Per issue as each finishes, and a batch report at the end — formats in `references/comment-templates.md`. Report only validation that actually ran, with its real output.

...
Unimplementable as written, or 3 failed verification cycles with no progress → post findings, skip, keep `dev` clean, continue the batch. Repo-level failure (auth, unresolvable repo) → stop the run and report. Everything else is covered by the rules above.

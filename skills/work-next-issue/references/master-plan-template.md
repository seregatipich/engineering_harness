...
3. **Environment & baseline** — what Step 2 provisioned (found current / brought up to date / set up from zero), any item that survived a real provisioning attempt as unprovisionable (the attempt made, why it failed, the exact commands the user runs to unblock), and the verification tiers this leaves achievable; then toolchain detection results (exact test/format/lint/typecheck commands, run recipe if any), the worktree bootstrap commands for implementation subagents, and every pre-existing failure found on clean `dev` — plus any tier that could not run and why. Pass criterion for the run: no NEW failures versus this baseline.

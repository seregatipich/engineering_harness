...
Git flow in this workspace is two-tier and uses DIRECT MERGE, not pull requests: branch feature/<name> off dev, commit AND push each logical chunk, merge finished tested work into dev, and promote dev -> main only when fully done. Commits directly to main/master, or branches created from them, are not part of this flow. "Done means merged" here means direct-merged (feature -> dev -> promote dev -> main); there are no pull requests to open or wait for.

Quality is driven through the installed plugins: superpowers test-driven-development before writing code, systematic-debugging on any failure, verification-before-completion before claiming done, and a codex review before merging.
EOF
